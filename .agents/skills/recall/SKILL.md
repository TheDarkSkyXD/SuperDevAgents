...
Use the active harness's workspace-scoped transcript or conversation-history interface. Never guess a global transcript path or search another workspace. If the harness does not expose transcripts, say so and continue with live repository and shared-record evidence.
...
3. Fan out across your chat history. Spawn parallel subagents on high reasoning with standard, non-fast model variants, each taking a slice of the corpus. Tell every subagent to order candidates by real modification time (`ls -t`) and never by UUID name, grep the topic first and then read only the matching chats and only their relevant regions, and skip the current chat plus obvious noise (subagent, eval, and test chats). Each returns the same schema, one block per chat: topic, the user's goal, decisions, open threads, struggles and corrections, and artifacts (PRs, tickets, branches), each citing the chat UUID. For one or two chats, skip the fan-out and search directly. The raw transcripts stay in the subagents. The main thread gets only their findings.

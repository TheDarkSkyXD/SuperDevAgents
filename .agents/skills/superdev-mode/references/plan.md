...
Produce a phased implementation plan grounded in the **Principles** section of the `superdev-mode` skill. The plan is the deliverable. Do not implement.
...
Read the **Principles** section of the `superdev-mode` skill end to end, and the leaf `principle-*` skills it indexes. The principles govern every plan decision; cross-link them.

## 2. Scope and constraints

State your read of scope and constraints in one paragraph. Use the harness's structured user-input tool only for genuinely ambiguous intent (the **never-block-on-the-human** principle skill); give concrete options with each open question.
...
- Pass `model:` explicitly per the configured roles (defaults `grok-4.6-fast-xhigh` for code, `claude-opus-5-5-max` for judgment).
...
If a phase creates or edits a skill, instruct the implementer to use the active harness's skill-authoring guidance and place the result under `.agents/skills/<skill-name>/`.
...
- Browser / Electron / Web UIs: the active harness's browser or desktop-control capability.
- CLIs and TUIs: the active harness's terminal or PTY-control capability.
...
In the overview, name which superdev-mode non-negotiables the implementer must apply, by name:

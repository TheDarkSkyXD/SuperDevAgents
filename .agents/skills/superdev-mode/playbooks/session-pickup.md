...
1. Locate the prior trail through the active harness's workspace-scoped transcript or conversation-history interface, a remote-agent URL, or a pushed branch. Never search another workspace. Read the metadata overview and last messages first, then scan back for decision points. Parse a long transcript in a subagent and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
